...
type EditorCan = ReturnType<Editor["can"]>
...
export function isEditorAlive(editor: Editor | null): editor is Editor {
  if (!editor || editor.isDestroyed) return false

  try {
    return !!editor.state && !!editor.view && !!editor.schema
  } catch {
    return false
  }
}
...
export function isEditorUsable(editor: Editor | null): editor is Editor {
  return isEditorAlive(editor) && editor.isEditable
}

export function safeCan(editor: Editor | null): EditorCan | null {
  if (!isEditorAlive(editor)) return null
...
export function safeIsActive(
  ...
  if (!isEditorAlive(editor)) return false
...
export function safeGetAttributes(
  editor: Editor | null,
  name: string
  ...
  try {
    return editor.getAttributes(name) as Record<string, unknown>
  } catch {
    return {}
  }
}
