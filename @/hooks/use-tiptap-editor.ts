...
import { isEditorAlive, safeCan } from "@/lib/tiptap-utils"
...
export function useTiptapEditor(providedEditor?: Editor | null): {
  ...
  useEffect(() => {
    if (!isEditorAlive(storageEditor)) return
  ...
  const editorState = useEditorState({
    ...
    selector(context) {
      if (!isEditorAlive(context.editor)) {
      ...
      try {
        return {
          ...
          canCommand: safeCan(context.editor)
            ? context.editor.can
            : undefined,
        }
      } catch {
