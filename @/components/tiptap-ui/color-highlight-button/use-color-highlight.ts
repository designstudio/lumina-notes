...
export function canColorHighlight(
  ...
  if (mode === "mark") {
    ...
    return can ? can.setMark("highlight") : false
  } else {
    if (!isExtensionAvailable(editor, ["nodeBackground"])) return false

    try {
      const can = safeCan(editor)
      return can ? can.toggleNodeBackgroundColor("test") : false
    } catch {
      return false
    }
  }
...
export function isColorHighlightActive(
  ...
  if (mode === "mark") {
    return highlightColor
      ? safeIsActive(editor, "highlight", { color: highlightColor })
      : safeIsActive(editor, "highlight")
  } else {
...
export function useColorHighlight(config: UseColorHighlightConfig) {
  ...
  const handleColorHighlight = useCallback(() => {
    if (!isEditorUsable(editor) || !canColorHighlightState || !actualColor || !label)
      return false
