...
export function canToggleBlockquote(
  ...
  if (!turnInto) {
    return can.toggleWrap("blockquote")
  }
  ...
  return can.toggleWrap("blockquote") || can.clearNodes()
}
...
export function toggleBlockquote(editor: Editor | null): boolean {
  ...
  try {
    ...
    const toggle = safeIsActive(editor, "blockquote")
...
export function useBlockquote(config?: UseBlockquoteConfig) {
  ...
  const isActive = safeIsActive(editor, "blockquote")
